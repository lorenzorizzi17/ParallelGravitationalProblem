...
Real Simulation::computeForces(bool saveEnergy){
    ...
    for(int i = 0; i < m_N; ++i) {
        for(int j = i + 1; j < m_N; ++j) {
            ...
            Real r2 = dx*dx + dy*dy + dz*dz + 1e-2f; // Add small term to avoid division by zero
            Real r = std::sqrt(r2);                  // CCompute square root once
