...
Real distanceMIC(Real pos1[3], Real pos2[3], Real L) {
    Real dx = *pos1 - *pos2;
...
Simulation::Simulation(int n_particles, Real timeStep, Real length) : m_N(n_particles), m_dt(timeStep), m_L(length) {
    ...
    for(int i = 0; i < m_N; ++i) {
        ...
        while(overlap) {
            for(int k = 0; k < 3; ++k) {
                temp_pos[k] = (static_cast<Real>(rand()) / RAND_MAX) * m_L;  // RANDOM ENGINE, as above, but should be ok
            }
            // Check for overlaps
            overlap = false;
            for(int j = 0; j < i; ++j) {
                Real other_pos[3] = {m_x[j], m_y[j], m_z[j]};
                Real d2 = distanceMIC(temp_pos, other_pos, m_L);
